...
public class TicTacToe {
    public static void main(String[] args) {
        ...
        while (quantity < 9) {
            quantity++;
            userInput(scan, chars);
            printCurrentState(chars);
            mySave = winCheck(chars);
            if (mySave.equals("X wins") || mySave.equals("O wins")) {
    ...
    static void userInput(Scanner scan, String[] chars) {
        String userMove = null;
        for (boolean userInputCheckBool = false; !userInputCheckBool; userInputCheckBool = userInputCheck(userMove, chars))
        {
    ...
    static boolean userInputCheck(String userMove, String[] chars) {
        ...
        if (userMoveArray[0].matches("[0-9]") && userMoveArray[1].matches("[0-9]")) {
            if (userMoveArray[0].matches("[1-3]") && userMoveArray[1].matches("[1-3]")) {
                switch (userMove) {
                    case "3 1":
                        return charsOccupiedCheck(6, chars);
                    case "2 1":
                        return charsOccupiedCheck(3, chars);
                    case "1 1":
                        return charsOccupiedCheck(0, chars);
                    case "3 2":
                        return charsOccupiedCheck(7, chars);
                    case "2 2":
                        return charsOccupiedCheck(4, chars);
                    case "1 2":
                        return charsOccupiedCheck(1, chars);
                    case "3 3":
                        return charsOccupiedCheck(8, chars);
                    case "2 3":
                        return charsOccupiedCheck(5, chars);
                    default:
                        return charsOccupiedCheck(2, chars);
                }
            } else {
    ...
    static boolean charsOccupiedCheck(int charNumber, String[] chars) {
    ...
    static void printCurrentState(String[] chars) {
        System.out.printf("---------%n| %s %s %s |%n| %s %s %s |%n| %s %s %s |%n---------%n", chars[0], chars[1], chars[2], chars[3], chars[4], chars[5], chars[6], chars[7], chars[8]);
    }
    ...
    static String winCheck(String[] chars) {
        ...
        else if (Arrays.asList(chars).contains("_")) return "Game not finished";
        else return "Draw";
    }

    static boolean aLotMore(String[] chars) {
        return Math.abs((Collections.frequency(Arrays.asList(chars), "X") - Collections.frequency(Arrays.asList(chars), "O"))) > 1;
    }
